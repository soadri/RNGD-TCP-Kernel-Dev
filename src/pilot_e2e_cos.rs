...
use rngd_tcp_kernel_dev::kernel::pilot_e2e_cos_kernel::{A, pilot_e2e_cos_kernel};

mod reference_data_e2e_cos;
use reference_data_e2e_cos::{CHECK_N, reference_a, reference_expected};
...
async fn main() {
    ...
    let _out_hbm = launch(pilot_e2e_cos_kernel, (&mut ctx, &input_hbm)).await;
    println!("Pilot E2E cos: kernel ran");
}
...
mod tests {
    ...
    async fn matches_actual_pytorch_output() {
        ...
        let out_hbm = launch(pilot_e2e_cos_kernel, (&mut ctx, &input_hbm)).await;
