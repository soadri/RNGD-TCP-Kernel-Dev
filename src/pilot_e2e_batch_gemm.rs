...
use rngd_tcp_kernel_dev::kernel::pilot_e2e_batch_gemm_kernel::{V, M, K, N, pilot_e2e_batch_gemm_kernel};

mod reference_data_e2e_batch_gemm;
use reference_data_e2e_batch_gemm::{reference_a, reference_b, reference_expected};
...
async fn main() {
    ...
    let _out_hbm = launch(pilot_e2e_batch_gemm_kernel, (&mut ctx, &a_hbm, &b_hbm)).await;
    println!("Pilot E2E batch_gemm: kernel ran");
}
...
mod tests {
    ...
    async fn matches_bf16_reference() {
        ...
        let out_hbm = launch(pilot_e2e_batch_gemm_kernel, (&mut ctx, &a_hbm, &b_hbm)).await;
