...
use rngd_tcp_kernel_dev::kernel::pilot_e2e_log_kernel::{A, pilot_e2e_log_kernel};

mod reference_data_e2e_log;
use reference_data_e2e_log::{CHECK_N, reference_input, reference_expected};
...
async fn main() {
    ...
    let _out_hbm = launch(pilot_e2e_log_kernel, (&mut ctx, &input_hbm)).await;
    println!("Pilot E2E log: kernel ran");
}
...
mod tests {
    ...
    async fn matches_actual_pytorch_output() {
        ...
        let out_hbm = launch(pilot_e2e_log_kernel, (&mut ctx, &input_hbm)).await;
