...
use rngd_tcp_kernel_dev::kernel::pilot_e2e_mul_kernel::{A, pilot_e2e_mul_kernel};

mod reference_data_e2e_mul;
use reference_data_e2e_mul::{CHECK_N, reference_a, reference_b, reference_expected};
...
async fn main() {
    ...
    let _out_hbm = launch(pilot_e2e_mul_kernel, (&mut ctx, &lhs_hbm, &rhs_hbm)).await;
    println!("Pilot E2E mul: kernel ran");
}
...
mod tests {
    ...
    async fn matches_actual_pytorch_output() {
        ...
        let out_hbm = launch(pilot_e2e_mul_kernel, (&mut ctx, &lhs_hbm, &rhs_hbm)).await;
