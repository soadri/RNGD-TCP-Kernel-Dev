...
use rngd_tcp_kernel_dev::kernel::pilot_e2e_tanh_kernel::{A, pilot_e2e_tanh_kernel};

mod reference_data_e2e_tanh;
use reference_data_e2e_tanh::{CHECK_N, reference_a, reference_expected};
...
async fn main() {
    ...
    let _out_hbm = launch(pilot_e2e_tanh_kernel, (&mut ctx, &input_hbm)).await;
    println!("Pilot E2E tanh: kernel ran");
}
...
mod tests {
    ...
    async fn matches_actual_pytorch_output() {
        ...
        let out_hbm = launch(pilot_e2e_tanh_kernel, (&mut ctx, &input_hbm)).await;
